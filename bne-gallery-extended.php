	...
	class BNE_GALLERY_EXTENDED {
	    ...
	    */
		function __construct() {
			...
			// Set Constants
			define( 'BNE_GALLERY_EXTENDED_VERSION', '1.0.3' );
		...
		function gallery_shortcode_hijack( $output = '', $atts, $instance ) {
			...
			*/
			if( $atts['display'] == 'carousel' || $atts['display'] == 'carrousel' ) {
				...
				// Output the gallery
				$output .= '<div id="bne-carousel-'.$rand_id.'" class="bne-gallery-carousel-wrapper clearfix">';
					...
					$output .= '<div class="slider-inner">';
						...
						// Loading icon
						$output .= '<div class="bne-loader"><i class="fa fa-spinner fa-spin"></i></div>';
						...
						$output .= '<ul class="carousel-slider gallery">';
							...
							foreach( $image_ids as $id ) {
								$output .= '<li class="slide gallery-item">';
									...
									if( $atts['caption'] == 'true' ) {
										...
										$output .= '<span class="caption">'.$meta['caption'].'</span>';
									}
			...
			*/
			if( $atts['display'] == 'masonry' ) {
				...
				// Initiate masonry.js on our gallery instance
				$output .= '<script type="text/javascript">
								jQuery(document).ready(function($) {
									$(window).load(function(){
				...
				// Output the gallery
				$output .= '<div id="bne-gallery-masonry-'.$rand_id.'" class="bne-gallery-masonry-wrapper gallery js-masonry">';
					...
					// Loop through Image Id's
					foreach( $image_ids as $id ) {
						$output .= '<div class="gallery-single gallery-item col-'.$atts['columns'].'-masonry" style="margin-bottom: '.$atts['gutter'].'px; width: '.$grid_col_width.';">';
		...
		*/
		function gallery_css() {
			...
			<style type="text/css">
				...
				.bne-gallery-carousel-wrapper .roundabout-nav a.prev { left: 5px; }
				.bne-gallery-carousel-wrapper .roundabout-nav a.next { right: 5px; }
				.bne-gallery-carousel-wrapper .roundabout-in-focus img { cursor: auto; }
				.bne-gallery-carousel-wrapper .caption { display: block; opacity: 0; text-align: center; padding: 8px; font-size: 12px; transition: opacity .3s ease; }
				.bne-gallery-carousel-wrapper .roundabout-in-focus .caption { opacity: 1; }
			
				/* == Masonry == */
				.bne-gallery-masonry-wrapper { margin-bottom: 10px; }
